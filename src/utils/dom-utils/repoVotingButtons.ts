...
import {
    checkUserVotedRepo,
    repoExistsOnKhulnaSoft,
} from "../fetchKhulnaSoftApiData";

const injectRepoVotingButtons = async (ownerName: string, repoName: string) => {
    if (document.getElementById("repo-voting-button") || !(await isLoggedIn()) || !(await repoExistsOnKhulnaSoft(ownerName, repoName))) {
        return;
    }
